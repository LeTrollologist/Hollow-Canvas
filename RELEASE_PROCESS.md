...
```bash
python scripts/pipeline.py v0.9.0
```
*or via Make:*
```bash
make release TAG=v0.9.0
```

### Local Build & Package Only (No Upload)
```bash
python scripts/pipeline.py v0.9.0 --no-publish
```

### Create as GitHub Draft Release
```bash
python scripts/pipeline.py v0.9.0 --draft
```
...
├── hollow-canvas-v0.9.0-windows-x86_64.zip         # Standard Zip distribution
├── hollow-canvas-v0.9.0-windows-x86_64.vpack       # VPack distribution
...
```bash
# Check SHA-256
certutil -hashfile hollow-canvas-v0.9.0-windows-x86_64.zip SHA256
